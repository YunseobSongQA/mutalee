...
- 구성: Cloudflare Pages(정적 + Functions) + 별도 발송 Worker(Durable Object 알람)

알림 발송은 Durable Object의 setAlarm이 담당해서 도래 시각 정각(±1초)에 나간다.
(예전 "1분 크론 + KV 폴링" 구조는 크론 오프셋 + KV 최종 일관성 때문에 최대 60초 늦었다.)
...
| `functions/api/sync.js` | 서버: 구독+규칙+프로필을 발송 Worker에 전달 (deviceId 기준, SYNC_TOKEN 인증) | `onRequestPost` |
| `functions/api/unsubscribe.js` | 서버: 구독 삭제를 발송 Worker에 전달 | `onRequestPost` |
| `worker/cron.js` | 발송 Worker: Durable Object가 구독을 저장하고 도래 시각 정각에 알람으로 웹푸시 발송. 크론은 알람 유실 대비 안전망. `reminders.js`의 순수 로직을 재사용 | `MutaleeScheduler` |
| `worker/wrangler.toml` | 발송 Worker 배포 설정 + Durable Object 바인딩 (Pages와 별도 배포라 따로 필요) | — |
...
| `VAPID_PRIVATE_KEY` | 발송 Worker 시크릿 | 웹푸시 서명 (공개키는 `data/push-config.json`) |
| `SYNC_TOKEN` | Pages와 발송 Worker 양쪽에 같은 값 | Functions→Worker 내부 호출 인증 (외부인의 발송 API 호출 차단) |
