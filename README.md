...
메인 파일(`index.html`, `app.js`, `style.css`)은 루트에 있고, 기능 코드는 `js/기능이름.js` 하나씩입니다.
`index.html`은 `app.js` 하나만 부르고, `app.js`가 `js/` 모듈들을 `import` 합니다. 전부 실제로 쓰이는 파일입니다.

| 파일 | 하는 일 | export |
|---|---|---|
| `index.html` | 앱 뼈대. `app.js` 하나만 부른다 | — |
| `app.js` | 화면 연결만 담당하는 진입점. 상태를 들고 각 기능을 조립한다 | — |
| `style.css` | 색·간격 변수 + 화면 스타일 전부 | — |
| `js/reminders.js` | 노티 기능 전부: 도래 계산·문구 템플릿(순수 로직) + localStorage/AI 요청 + 목록·폼·모달 화면 | `getTodaysReminders`, `loadRules`, `renderReminderList` 등 |
| `js/culture.js` | 오늘의 교양 전부: 날짜 기반 선택(순수 로직) + 하루 유지/AI 요청 + 카드 화면 | `pickDaily`, `generateCulture`, `renderCultureCard` 등 |
| `js/notify.js` | 알림 기능 전부: 권한·앱 내 스위치·포그라운드 알림·웹푸시 구독 + 상태 배너 화면 | `checkAndNotify`, `subscribe`, `renderNoticeBanner` 등 |
...
| `worker/cron.js` | 1분마다 KV의 구독을 훑어 도래한 노티를 웹푸시 발송. `js/reminders.js`의 순수 로직을 재사용 | — |
